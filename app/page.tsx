...
interface Budget { id: number; amount: number; spent: number; category_id: number; category_name: string; category_color: string; }
interface SavingsGoal { id: number; name: string; icon: string; color: string; target_amount: number; current_amount: number; deadline: string | null; status: string; }
...
function AnimatedNumber({ value, formatter }: { value: number; formatter: (n: number) => string }) {
  ...
  useEffect(() => {
    ...
    const start = prevVal.current; const diff = value - start;
...
const STAT_CONFIG = [
  ...
  { key: 'savings',    label: 'Saved',       color: '#5b6ef5', bg: 'rgba(91,110,245,0.08)',  border: 'rgba(91,110,245,0.15)', icon: '◈' },
  { key: 'monthlyNet', label: 'Monthly Net', color: '#f5a623', bg: 'rgba(245,166,35,0.08)',  border: 'rgba(245,166,35,0.15)', icon: '◎' },
];
...
function TiltStatCard({ cfg, val, incomeVal, staggerIdx, fmt }: { cfg: typeof STAT_CONFIG[0]; val: number; incomeVal: number; staggerIdx: number; fmt: (n: number) => string }) {
  ...
  const pct = cfg.key === 'income' ? 100 : incomeVal > 0 ? Math.min(100, Math.max(0, (Math.abs(val) / incomeVal) * 100)) : 0;
  return (
    <div ref={ref} className={`tilt-card animate-fadeUp stagger-${staggerIdx + 1}`}
      onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}
      style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <span style={{ width: 22, height: 22, borderRadius: 6, background: cfg.bg, border: `1px solid ${cfg.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: cfg.color, fontWeight: 700 }}>{cfg.icon}</span>
      </div>
      <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: '-0.04em', color: cfg.key === 'monthlyNet' ? (val >= 0 ? cfg.color : '#f05252') : cfg.color, lineHeight: 1 }}>
...
function BudgetOverviewStrip({ budgets, fmt }: { budgets: Budget[]; fmt: (n: number) => string }) {
  if (budgets.length === 0) return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center', padding: '16px 0', textAlign: 'center' }}>
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.4 }}><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>No budgets set for this month</div>
      <a href="/dashboard/budgets" style={{ fontSize: 12, color: 'var(--accent)', textDecoration: 'none', fontWeight: 700, padding: '6px 14px', borderRadius: 8, background: 'rgba(91,110,245,0.1)', border: '1px solid rgba(91,110,245,0.2)' }}>Set budgets →</a>
    </div>
  );
  ...
  const warnCount = budgets.filter(b => { const p = (Number(b.spent) / Number(b.amount)) * 100; return p >= 80 && p < 100; }).length;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-muted)', marginBottom: 5 }}>
            <span>{fmt(totalSpent)} spent of {fmt(totalBudget)}</span>
          ...
          <div style={{ height: 6, borderRadius: 99, background: 'var(--surface-2)', overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: 99, width: `${overallPct}%`, background: overallPct >= 100 ? 'var(--red)' : overallPct >= 80 ? 'var(--amber)' : 'var(--green)', transition: 'width 0.9s cubic-bezier(0.34,1.1,0.64,1)', boxShadow: `0 0 6px ${overallPct >= 80 ? 'rgba(240,82,82,0.4)' : 'rgba(34,212,122,0.3)'}` }} />
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          {overCount > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--red)', background: 'rgba(240,82,82,0.1)', padding: '3px 8px', borderRadius: 6 }}>🚨 {overCount} over</span>}
          {warnCount > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--amber)', background: 'rgba(245,166,35,0.1)', padding: '3px 8px', borderRadius: 6 }}>⚠ {warnCount} near limit</span>}
          {overCount === 0 && warnCount === 0 && <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--green)', background: 'rgba(34,212,122,0.1)', padding: '3px 8px', borderRadius: 6 }}>✓ All on track</span>}
        </div>
      </div>
      {/* Category bars — top 4 only */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 6 }}>
        {budgets.slice(0, 4).map(b => {
          ...
          return (
            <div key={b.id} style={{ padding: '8px 10px', background: 'var(--surface-2)', borderRadius: 9, border: `1px solid ${over ? 'rgba(240,82,82,0.2)' : 'var(--border)'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: b.category_color, flexShrink: 0 }} />
                  <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-soft)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 80 }}>{translateCategory(b.category_name)}</span>
                </div>
                <span style={{ fontSize: 10, fontWeight: 700, color: over ? 'var(--red)' : warn ? 'var(--amber)' : 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>{pct.toFixed(0)}%</span>
              </div>
      ...
      {budgets.length > 4 && (
        <div style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'right' }}>
          ...
          <a href="/dashboard/budgets" style={{ color: 'var(--accent)', textDecoration: 'none', fontWeight: 600 }}>View all →</a>
        </div>
      )}
    </div>
  );
...
function SavingsOverviewStrip({ goals, fmt }: { goals: SavingsGoal[]; fmt: (n: number) => string }) {
  ...
  if (goals.length === 0) return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center', padding: '16px 0', textAlign: 'center' }}>
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.4 }}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>No savings goals yet</div>
      <a href="/dashboard/savings" style={{ fontSize: 12, color: 'var(--accent)', textDecoration: 'none', fontWeight: 700, padding: '6px 14px', borderRadius: 8, background: 'rgba(91,110,245,0.1)', border: '1px solid rgba(91,110,245,0.2)' }}>Create a goal →</a>
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ height: 6, borderRadius: 99, background: 'var(--surface-2)', overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: 99, width: `${overallPct}%`, background: 'var(--accent)', transition: 'width 0.9s cubic-bezier(0.34,1.1,0.64,1)', boxShadow: '0 0 6px rgba(91,110,245,0.35)' }} />
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {active.slice(0, 3).map(g => {
          ...
          return (
            <div key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'var(--surface-2)', borderRadius: 10, border: `1px solid ${g.color}25`, flex: '1 1 160px', minWidth: 0 }}>
              <div style={{ width: 28, height: 28, borderRadius: 8, background: g.color + '20', border: `1.5px solid ${g.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0 }}>{g.icon}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 11, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 3 }}>{g.name}</div>
        ...
        {active.length > 3 && (
          <a href="/dashboard/savings" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 12px', background: 'var(--surface-2)', borderRadius: 10, border: '1px solid var(--border)', fontSize: 11, color: 'var(--text-muted)', textDecoration: 'none', flexShrink: 0 }}>
...
export default function DashboardPage() {
  ...
  const handleQuickSave = async () => {
    ...
    try {
      ...
      showToast('Transaction added'); closeModal(); load();
  ...
  const addRipple = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    dot.style.left = `${e.clientX - rect.left - 5}px`; dot.style.top = `${e.clientY - rect.top - 5}px`;
  ...
  const statValues: Record<string, number> = {
    income: stats?.income ?? 0, expenses: stats?.expenses ?? 0,
    savings: stats?.savings ?? 0, monthlyNet,
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="animate-fadeUp" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={e => { addRipple(e); openQuickAdd(); }} className="btn-ripple"
            style={{ padding: '10px 16px', borderRadius: 10, fontSize: 13, fontWeight: 700, background: 'var(--accent)', color: 'white', border: 'none', boxShadow: '0 4px 16px rgba(91,110,245,0.3)', whiteSpace: 'nowrap' }}>
            + Add
          </button>
        </div>
      </div>
        ...
        {loading
          ? STAT_CONFIG.map((s, i) => (
            <div key={s.key} className={`animate-fadeUp stagger-${i + 1}`} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16 }}>
              <Skeleton w={64} h={10} /><div style={{ marginTop: 12 }}><Skeleton w={100} h={20} /></div><div style={{ marginTop: 10 }}><Skeleton h={4} /></div>
            </div>
          ))
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 0, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', flexWrap: 'wrap' }}>
          ...
          {totalSavedInGoals > 0 && (
            <div style={{ padding: '10px 16px', borderRight: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
          ...
          <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--green)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Available</span>
            <span style={{ fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-mono)', color: availableBalance >= 0 ? 'var(--green)' : 'var(--red)' }}>{fmt(availableBalance)}</span>
      ...
      {/* ── Quick Add Modal ── */}
      {showQuickAdd && (
        <div className="modal-overlay" style={MODAL_OVERLAY} onClick={e => { if (e.target === e.currentTarget) closeModal(); }}>
          <div className={`modal-box ${modalExiting ? 'modal-box-exit' : 'modal-box-enter'}`}
            style={{ width: '100%', maxWidth: 480, borderRadius: '20px 20px 0 0', background: 'var(--surface)', border: '1px solid var(--border-2)', padding: '20px 20px 32px', boxShadow: '0 -8px 48px rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button onClick={closeModal} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text-muted)', fontSize: 16, lineHeight: 1, padding: '4px 8px', borderRadius: 7 }}>×</button>
            </div>
            <div style={{ display: 'flex', background: 'var(--surface-2)', borderRadius: 10, padding: 4, border: '1px solid var(--border)', position: 'relative' }}>
              <div style={{ position: 'absolute', top: 4, bottom: 4, width: 'calc(50% - 4px)', left: form.type === 'expense' ? 4 : 'calc(50%)', borderRadius: 7, background: form.type === 'income' ? 'var(--green)' : 'var(--red)', transition: 'left 0.28s cubic-bezier(0.34,1.1,0.64,1), background 0.22s ease', pointerEvents: 'none' }} />
              {['expense', 'income'].map(t => (
                <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))} style={{ flex: 1, padding: '9px', borderRadius: 7, fontSize: 13, fontWeight: 600, background: 'transparent', color: form.type === t ? 'white' : 'var(--text-muted)', border: 'none', textTransform: 'capitalize', position: 'relative', zIndex: 1, transition: 'color 0.2s ease' }}>{t}</button>
              ))}
            </div>
            <div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={closeModal} style={{ flex: 1, padding: '12px', borderRadius: 10, fontSize: 13, fontWeight: 600, background: 'var(--surface-2)', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>Cancel</button>
              <button onClick={handleQuickSave} disabled={saving} className="btn-ripple" onMouseDown={addRipple} style={{ flex: 1, padding: '12px', borderRadius: 10, fontSize: 13, fontWeight: 700, background: 'var(--accent)', color: 'white', border: 'none', boxShadow: '0 4px 16px rgba(91,110,245,0.3)', opacity: saving ? 0.6 : 1 }}>
                {saving ? 'Saving…' : 'Save'}
              </button>
            </div>
          </div>
