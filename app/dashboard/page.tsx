...
function TiltStatCard({ cfg, val, incomeVal, staggerIdx, fmt }: { cfg: typeof STAT_CONFIG[0]; val: number; incomeVal: number; staggerIdx: number; fmt: (n: number) => string }) {
  ...
  return (
    <div ref={ref} className={`tilt-card animate-fadeUp stagger-${staggerIdx + 1}`}
      ...
      style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px', position: 'relative', overflow: 'hidden' }}
    >
      <div className="tilt-glow" />
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <span className="stat-card-label" style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.03em', textTransform: 'uppercase' }}>{cfg.label}</span>
      ...
      <div className="stat-card-value" style={{ fontSize: 18, fontWeight: 700, letterSpacing: '-0.04em', color: cfg.color, lineHeight: 1 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="animate-fadeUp" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <h1 className="page-title" style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.04em', lineHeight: 1.2, marginBottom: 4 }}>
            {MONTHS[month-1]} {year}
          </h1>
          <p className="page-subtitle" style={{ fontSize: 13, color: 'var(--text-muted)', letterSpacing: '-0.01em' }}>
            Financial overview
        ...
        <div className="header-actions" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{ padding: '10px 16px', borderRadius: 10, fontSize: 13, fontWeight: 700, background: 'var(--accent)', color: 'white', border: 'none', boxShadow: '0 4px 16px rgba(91,110,245,0.3)', letterSpacing: '-0.01em', whiteSpace: 'nowrap', transition: 'all 0.18s ease' }}
      ...
      <div className="stats-grid-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {loading ? STAT_CONFIG.map((s, i) => (
          <div key={s.key} className={`animate-fadeUp stagger-${i+1}`} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16 }}>
            <Skeleton w={64} h={10} /><div style={{ marginTop: 12 }}><Skeleton w={100} h={20} /></div>
            <div style={{ marginTop: 10 }}><Skeleton h={4} /></div>
          </div>
          ...
          {loading ? <Skeleton h={180} /> : (
            <ResponsiveContainer key={chartKey} width="100%" height={180}>
              <AreaChart data={trendData} margin={{ left: -10, right: 0, top: 4, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="income" stroke="#22d47a" strokeWidth={2} fill="url(#gI)" name="Income" dot={false} isAnimationActive={true} animationDuration={900} animationEasing="ease-out" />
                <Area type="monotone" dataKey="expenses" stroke="#f05252" strokeWidth={2} fill="url(#gE)" name="Expenses" dot={false} isAnimationActive={true} animationDuration={900} animationEasing="ease-out" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {loading ? (
            ...
            <>
              <ResponsiveContainer key={chartKey} width="100%" height={130}>
                <PieChart>
                  <Pie data={categoryData} dataKey="total" nameKey="name" cx="50%" cy="50%" innerRadius={34} outerRadius={56} paddingAngle={3} startAngle={90} endAngle={-270} isAnimationActive={true} animationDuration={800} animationEasing="ease-out">
